...
def remove_rare_labels(df, var, frequent_ls, replacement='Rare'):
    # groups labels that are not in the frequent list into the umbrella
    # group Rare
    return np.where(df[var].isin(frequent_ls), df[var], replacement)
...
def scale_features(df, output_path):
    ...
    return pd.DataFrame(model.transform(df), columns=df.columns)
